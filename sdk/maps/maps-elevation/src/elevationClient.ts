...
import {
  ElevationClientOptionalParams,
  JsonFormat,
  ElevationClientGetDataForPointsOptionalParams,
  ElevationClientGetDataForPointsResponse,
  LatLongPairAbbreviated,
  ElevationClientPostDataForPointsOptionalParams,
  ElevationClientPostDataForPointsResponse,
  ElevationClientGetDataForPolylineOptionalParams,
  ElevationClientGetDataForPolylineResponse,
  ElevationClientPostDataForPolylineOptionalParams,
  ElevationClientPostDataForPolylineResponse,
  ElevationClientGetDataForBoundingBoxOptionalParams,
  ElevationClientGetDataForBoundingBoxResponse
} from "./models";
...
export class ElevationClient extends ElevationClientContext {
  ...
  getDataForPoints(
    format: JsonFormat,
    points: string[],
    options?: ElevationClientGetDataForPointsOptionalParams
  ): Promise<ElevationClientGetDataForPointsResponse> {
    return this.sendOperationRequest({ format, points, options }, getDataForPointsOperationSpec);
  }
   ...
   *
   *  Use the POST endpoint only if you intend to pass multiple points in the request. If you intend to
   * pass  a single coordinate into the API, use the [GET Data For Points
   * API](https://docs.microsoft.com/rest/api/maps/elevation/getdataforpoints).
   *
   *  The result will be in the same sequence of points listed in the request.
   * @param format Desired format of the response. Only `json` format is supported.
   * @param points The string representation of a list of points. A point is defined in lon/lat WGS84
   *               coordinate reference system format.  Each points in a list should be separated by the pipe ('|')
   *               character. The number of points that can be requested in  a POST request ranges from 2 to 2,000. The
   *               resolution of the elevation data will be the highest for a single point  and will decrease if
  ...
  postDataForPoints(
    ...
    options?: ElevationClientPostDataForPointsOptionalParams
  ): Promise<ElevationClientPostDataForPointsResponse> {
    return this.sendOperationRequest({ format, points, options }, postDataForPointsOperationSpec);
  }
   ...
   * that will be used to divide polyline into equally spaced segments.
   *
   *  Elevation data at both start and endpoints, as well as equally spaced points along the polyline
   * will be returned. The results will be listed in the direction from the first endpoint towards the
   * last endpoint. A line between two endpoints is a straight Cartesian line, the  shortest line between
  ...
  getDataForPolyline(
    format: JsonFormat,
    lines: string[],
    options?: ElevationClientGetDataForPolylineOptionalParams
  ): Promise<ElevationClientGetDataForPolylineResponse> {
    return this.sendOperationRequest({ format, lines, options }, getDataForPolylineOperationSpec);
  }
   ...
   * that will be used to divide polyline into equally spaced segments.
   *
   *  Elevation data at both start and end points, as well as equally spaced  points along the polyline
   * will be returned. The results will be listed in the direction from the first endpoint towards the
   * last endpoint. A line between two endpoints is a straight Cartesian line, the  shortest line between
   * those two points in the coordinate reference system. Note that the point is  chosen based on
   * Euclidean distance and may markedly differ from the geodesic path along the curved surface of the
   * reference ellipsoid.
   * @param format Desired format of the response. Only `json` format is supported.
   * @param polyline The string representation of a polyline path. A polyline is defined by endpoint
   *                 coordinates,  with each endpoint separated by a pipe ('|') character. The polyline should be defined
   *                 in the  following format: `[longitude_point1, latitude_point1 | longitude_point2, latitude_point2,
   *                 ...,  longitude_pointN, latitude_pointN]`. The longitude and latitude values refer to the World
   *                 Geodetic  System (WGS84) coordinate reference system. The resolution of the data used to compute the
   *                  elevation will depend on the distance between the endpoints.
   * @param options The options parameters.
   */
  postDataForPolyline(
    format: JsonFormat,
    polyline: LatLongPairAbbreviated[],
    options?: ElevationClientPostDataForPolylineOptionalParams
  ): Promise<ElevationClientPostDataForPolylineResponse> {
    return this.sendOperationRequest(
      { format, polyline, options },
   ...
   *             number of vertices (rows x columns) in the grid should be less than 2,000.
  ...
  getDataForBoundingBox(
    ...
    rows: number,
    columns: number,
    options?: ElevationClientGetDataForBoundingBoxOptionalParams
  ): Promise<ElevationClientGetDataForBoundingBoxResponse> {
    return this.sendOperationRequest(
...
const getDataForPointsOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      bodyMapper: Mappers.ElevationResult
    },
    default: {
...
const postDataForPointsOperationSpec: coreClient.OperationSpec = {
  ...
  requestBody: Parameters.points1,
  queryParameters: [Parameters.apiVersion],
  urlParameters: [Parameters.$host, Parameters.format],
  headerParameters: [Parameters.accept, Parameters.clientId, Parameters.contentType],
  mediaType: "json",
  serializer
};
const getDataForPolylineOperationSpec: coreClient.OperationSpec = {
  ...
  queryParameters: [Parameters.apiVersion, Parameters.lines, Parameters.samples],
  urlParameters: [Parameters.$host, Parameters.format],
  headerParameters: [Parameters.accept, Parameters.clientId],
  serializer
};
const postDataForPolylineOperationSpec: coreClient.OperationSpec = {
  ...
  requestBody: Parameters.polyline,
...
const getDataForBoundingBoxOperationSpec: coreClient.OperationSpec = {
  ...
  queryParameters: [Parameters.apiVersion, Parameters.bounds, Parameters.rows, Parameters.columns],
  urlParameters: [Parameters.$host, Parameters.format],
  headerParameters: [Parameters.accept, Parameters.clientId],
  serializer
};
