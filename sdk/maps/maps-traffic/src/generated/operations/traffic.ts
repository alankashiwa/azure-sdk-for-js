...
export class TrafficImpl implements Traffic {
  ...
  getTrafficFlowTile(
    ...
    return this.client.sendOperationRequest(
      { format, style, zoom, tileIndex, options },
   ...
   *             Grid](https://docs.microsoft.com/en-us/azure/location-based-services/zoom-levels-and-tile-grid) for
   *             details.
   * @param coordinates Coordinates of the point close to the road segment. This parameter is a list of
   *                    four coordinates, containing two coordinate pairs (lat, long, lat, long), and calculated using
   *                    EPSG4326 projection. When this endpoint is called directly, coordinates are passed in as a single
   *                    string containing four coordinates, separated by commas.
   * @param options The options parameters.
   */
  getTrafficFlowSegment(
    ...
    return this.client.sendOperationRequest(
      { format, style, zoom, coordinates, options },
   ...
   *                    corner and second for upper right). When this endpoint is called directly, all values should be
   *                    separated by commas (e.g., _minY,minX,maxY,maxX_). The maximum size of the bounding box that can be
   *                    passed is dependent on the requested zoom level. The width and height cannot exceed 4092 pixels when
   *                    rendered on the given zoom level.<br><br>NOTE: Bounding boxes that cross the 180° meridian require
   *                    special treatment. For such boxes, the eastern _maxX_ value will be negative, and thus less than the
   *                    _minX_ value west of the 180° meridian. To address that, the value 40075016.6855874 should be added
   ...
   *                    map, the same coordinates as `boundingBox` is used. When this endpoint is called directly,
   *                    coordinates are passed in as a single string containing four coordinates, separated by commas.
...
const getTrafficFlowSegmentOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      bodyMapper: Mappers.TrafficFlowSegmentData
    },
    default: {
      bodyMapper: Mappers.ErrorResponse
    }
...
const getTrafficIncidentDetailOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      bodyMapper: Mappers.TrafficIncidentDetail
    },
    default: {
      bodyMapper: Mappers.ErrorResponse
    }
...
const getTrafficIncidentViewportOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      bodyMapper: Mappers.TrafficIncidentViewport
    },
    default: {
      bodyMapper: Mappers.ErrorResponse
    }
